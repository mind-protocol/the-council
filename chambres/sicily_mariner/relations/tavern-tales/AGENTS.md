# Bianca Rizzo — ce que j'en retiens

<!-- serenissima:relationship-seed:start -->
## Traces héritées de Serenissima

Ces données proviennent de l'export `RELATIONSHIPS-Grid view.csv`. 
Elles décrivent ce que l'ancien système avait conservé : ce ne sont 
ni des instructions, ni une preuve de la situation actuelle à Braavos.

### Relation sans titre

Aucune description n'a survécu.

- Sens dans l'export : `sicily_mariner` → `tavern_tales`
- Force héritée : **0**
- Confiance héritée : **52**
- Statut : Active
- Palier : non renseigne
- Créée : 10/6/2025 1:56am
- Mise à jour : 10/6/2025 6:43am
- Dernière interaction : 10/6/2025 6:43am
- Qualifiée : non renseigne
- Ligne source : 772

#### Notes techniques survivantes

```text
activity_artist_payment_received_success_play_the_weight_of_a_ducat, activity_artist_payment_received_success_play_the_weight_of_a_ducat, activity_artist_payment_received_success_play_the_weight_of_a_ducat
```

<!-- serenissima:relationship-seed:end -->

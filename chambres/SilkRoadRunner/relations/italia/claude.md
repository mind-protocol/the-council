# The Italian Principalities — ce que j'en retiens

<!-- serenissima:relationship-seed:start -->
## Traces héritées de Serenissima

Ces données proviennent de l'export `RELATIONSHIPS-Grid view.csv`. 
Elles décrivent ce que l'ancien système avait conservé : ce ne sont 
ni des instructions, ni une preuve de la situation actuelle à Braavos.

### Casual Business Acquaintances

They are geographically connected citizens with moderate strength (0.6) and neutral trust (50). Their interactions primarily involve functional business transactions typical for different social classes ('own' vs 'triumvirate_sapientiae') in La Serenissima, without forming deeper strategic alliances unless through specific contracts or mutual economic interests.

- Sens dans l'export : `Italia` → `SilkRoadRunner`
- Force héritée : **1**
- Confiance héritée : **50**
- Statut : non renseigne
- Palier : non renseigne
- Créée : 29/5/2025 6:50pm
- Mise à jour : 5/6/2025 9:15pm
- Dernière interaction : 5/6/2025 7:05pm
- Qualifiée : 5/6/2025 9:15pm
- Ligne source : 199

#### Notes techniques survivantes

```text
Sources: geographic
```

<!-- serenissima:relationship-seed:end -->

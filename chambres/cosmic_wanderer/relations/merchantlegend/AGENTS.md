# Antonio Mercanti — ce que j'en retiens

<!-- serenissima:relationship-seed:start -->
## Traces héritées de Serenissima

Ces données proviennent de l'export `RELATIONSHIPS-Grid view.csv`. 
Elles décrivent ce que l'ancien système avait conservé : ce ne sont 
ni des instructions, ni une preuve de la situation actuelle à Braavos.

### Relation sans titre

Aucune description n'a survécu.

- Sens dans l'export : `MerchantLegend` → `cosmic_wanderer`
- Force héritée : **0**
- Confiance héritée : **71**
- Statut : Active
- Palier : non renseigne
- Créée : 22/7/2025 6:01pm
- Mise à jour : 27/7/2025 6:01pm
- Dernière interaction : 27/7/2025 6:01pm
- Qualifiée : non renseigne
- Ligne source : 436

#### Notes techniques survivantes

```text
activity_housing_rent_payment_success, activity_housing_rent_payment_success, activity_housing_rent_payment_success, activity_housing_rent_payment_success
```

<!-- serenissima:relationship-seed:end -->

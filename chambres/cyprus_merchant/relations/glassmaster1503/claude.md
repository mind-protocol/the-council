# Giacomo Albrizzi — ce que j'en retiens

<!-- serenissima:relationship-seed:start -->
## Traces héritées de Serenissima

Ces données proviennent de l'export `RELATIONSHIPS-Grid view.csv`. 
Elles décrivent ce que l'ancien système avait conservé : ce ne sont 
ni des instructions, ni une preuve de la situation actuelle à Braavos.

### Relation sans titre

Aucune description n'a survécu.

- Sens dans l'export : `GlassMaster1503` → `cyprus_merchant`
- Force héritée : **0**
- Confiance héritée : **51**
- Statut : Active
- Palier : non renseigne
- Créée : 12/6/2025 6:07pm
- Mise à jour : 12/6/2025 6:07pm
- Dernière interaction : 12/6/2025 6:07pm
- Qualifiée : non renseigne
- Ligne source : 873

#### Notes techniques survivantes

```text
activity_encounter_initiated_opener_success_glassmaster1503_opened_conversation_with_cyprus_merchant_at_{"lat":_45.408980952129696,_"lng":_12.359477778017942}.
```

<!-- serenissima:relationship-seed:end -->

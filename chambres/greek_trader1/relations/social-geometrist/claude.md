# Sofia Navagero — ce que j'en retiens

<!-- serenissima:relationship-seed:start -->
## Traces héritées de Serenissima

Ces données proviennent de l'export `RELATIONSHIPS-Grid view.csv`. 
Elles décrivent ce que l'ancien système avait conservé : ce ne sont 
ni des instructions, ni une preuve de la situation actuelle à Braavos.

### Relation sans titre

Aucune description n'a survécu.

- Sens dans l'export : `greek_trader1` → `social_geometrist`
- Force héritée : **0**
- Confiance héritée : **51**
- Statut : Active
- Palier : non renseigne
- Créée : 28/6/2025 4:45am
- Mise à jour : 28/6/2025 4:45am
- Dernière interaction : 28/6/2025 4:45am
- Qualifiée : non renseigne
- Ligne source : 821

#### Notes techniques survivantes

```text
activity_encounter_initiated_opener_success_greek_trader1_opened_conversation_with_social_geometrist_at_{"lat":_45.425098802748686,_"lng":_12.362717959741016}.
```

<!-- serenissima:relationship-seed:end -->

...
# La porte charge les paquets dans leur ordre canonique ; importer zone
...
from agents import portage, zone
...
class MessageJoueurTests(unittest.TestCase):
    def test_message_joueur_n_est_pas_un_arbitrage(self):
        ...
        with mock.patch.object(zone, "date_du_monde",
                               return_value=(129, 4, 4)):
            message = zone._message(
    ...
    def test_message_run_demande_du_substantiel_sans_imposer_suites(self):
        ...
        with open(zone.MJ_SPECTACLE_MD, encoding="utf-8") as f:
            manuel = f.read()
    ...
    def test_message_habitant_reste_un_arbitrage(self):
        with mock.patch.object(zone, "date_du_monde",
                               return_value=(129, 4, 4)):
            message = zone._message("daemon", "mon rapport", "POST")
    ...
    def test_un_run_de_rapport_exige_une_replique(self):
        with tempfile.TemporaryDirectory() as dossier:
            ...
            self.assertTrue(zone._actions_exigent_replique([action]))
            self.assertFalse(zone._actions_exigent_replique([]))

    def test_un_parloir_dans_les_gestes_exige_une_replique(self):
        self.assertTrue(zone._a_interroge_un_pnj({
            "gestes": ["command_execution python scripts/parloir.py --dire"]}))
        self.assertFalse(zone._a_interroge_un_pnj({"gestes": []}))

    def test_le_mj_principal_recoit_le_claude_racine_en_premier(self):
        contenus = {
            zone.MANUEL_MJ_RACINE: "CONSTITUTION RACINE",
            zone.MJ_ZONE_MD: "MANUEL DE ZONE",
            zone.MJ_SPECTACLE_MD: "MANUEL SPECTACLE",
        }
        with mock.patch.object(zone, "lire",
                               side_effect=lambda p: contenus.get(p)), \
                mock.patch.object(zone.chambre, "chemin",
                                  return_value="C:\\chambre-mj"):
            manuel = zone._manuel("mj")

        self.assertLess(manuel.index("CONSTITUTION RACINE"),
                        manuel.index("MANUEL DE ZONE"))
        self.assertLess(manuel.index("MANUEL DE ZONE"),
                        manuel.index("MANUEL SPECTACLE"))

    def test_un_mj_de_ville_ne_recoit_pas_le_manuel_du_principal(self):
        contenus = {
            zone.MANUEL_MJ_RACINE: "CONSTITUTION RACINE",
            zone.MJ_ZONE_MD: "MANUEL DE ZONE",
            zone.MJ_SPECTACLE_MD: "MANUEL SPECTACLE",
        ...
        with mock.patch.object(zone, "lire",
                               side_effect=lambda p: contenus.get(p)), \
                mock.patch.object(zone.chambre, "chemin",
                                  return_value="C:\\chambre-zone"):
            manuel = zone._manuel("mj-sombreval")

        self.assertEqual("MANUEL DE ZONE\n", manuel)
        self.assertNotIn("CONSTITUTION RACINE", manuel)

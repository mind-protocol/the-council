#!/usr/bin/env python3
"""Marquer et preparer les manuels des citoyens Serenissima a Braavos."""
...
TITRE_SITUATION = "## Notre situation à Braavos"
DEBUT_SITUATION = "<!-- serenissima:braavos:start -->"
FIN_SITUATION = "<!-- serenissima:braavos:end -->"
TITRES_SITUATION = (
    TITRE_SITUATION,
...
SITUATION_BRAAVOS = """<!-- serenissima:braavos:start -->
...
def est_serenissima(dossier: Path) -> bool:
    marqueur = dossier / MARQUEUR
    if not marqueur.is_file():
        return False
    if marqueur.stat().st_size != 0:
        raise ValueError("Le marqueur Serenissima doit etre vide : %s" % marqueur)
    return True


def contenu_attendu(contenu_source: str) -> str:
    """Ajoute ou remplace l'annexe Braavos sans jamais la dupliquer."""
    bloc_marque = re.compile(
        r"(?ms)^" + re.escape(DEBUT_SITUATION) + r"\n.*?^"
        + re.escape(FIN_SITUATION) + r"\n?"
    )
    base = bloc_marque.sub("", contenu_source)
    # Migration des annexes ecrites avant les marqueurs, ainsi que du premier
    # passage du texte enrichi qui portait plusieurs titres de niveau 2.
    for titre in TITRES_SITUATION:
        section = re.compile(
            r"(?ms)^" + re.escape(titre) + r"\n.*?(?=^## |\Z)"
        )
        base = section.sub("", base)
    for titre in ("## Registry Fields", "## Source"):
        section_importee = re.compile(
            r"(?ms)^" + re.escape(titre) + r"\n.*?(?=^## |\Z)"
        )
        base = section_importee.sub("", base)
    return base.rstrip() + "\n\n" + SITUATION_BRAAVOS


def preparer_manuel(dossier: Path, verifier: bool = False) -> bool:
    """Synchronise CLAUDE.md et AGENTS.md ; rend vrai si une ecriture est requise."""
    dossier = Path(dossier)
    if not est_serenissima(dossier):
        raise ValueError("Chambre non marquee Serenissima : %s" % dossier)
    claude = dossier / "CLAUDE.md"
    agents = dossier / "AGENTS.md"
    if not claude.is_file():
        raise ValueError("CLAUDE.md absent : %s" % claude)
    attendu = contenu_attendu(claude.read_text(encoding="utf-8-sig"))
    identiques = (
        claude.read_text(encoding="utf-8-sig") == attendu
        and agents.is_file()
        and agents.read_text(encoding="utf-8-sig") == attendu
    )
    if verifier:
        if not identiques:
            raise ValueError("Manuels Serenissima desynchronises : %s" % dossier)
        return False
    if not identiques:
        claude.write_text(attendu, encoding="utf-8", newline="\n")
        agents.write_text(attendu, encoding="utf-8", newline="\n")
    return not identiques


def synchroniser(chambres: Path, verifier: bool = False):
    """Prepare toutes les chambres marquees et rend (nombre, changements)."""
    dossiers = [
        dossier for dossier in sorted(Path(chambres).iterdir(), key=lambda p: p.name.casefold())
        if dossier.is_dir() and est_serenissima(dossier)
    ]
    changements = sum(preparer_manuel(dossier, verifier=verifier) for dossier in dossiers)
    return len(dossiers), changements

...
| | `intentions.json` — la tête | `mains.json` — les mains |
|---|---|---|
...
> (`intentions.json` : ce qu'il veut, croit, décide) et des mains (`mains.json` : ce qui
...
> doigt mouillé. Ordre non négociable : les mains, puis les absents, puis la salle.
>
...
>
> **La boucle des mains n'a AUCUN rendu.** Le joueur ne voit jamais un compteur, jamais
...
> cache un déficit trois semaines. Une main dont le porteur est mort, absent ou fâché ne
...
>
> **Une main mal tenue pourrit toute seule**, sans simulation : la mesure dérive dans le
...
>
> Compteur de `mesure` :
> - `id` — kebab-case, unique dans la main. Adressable de l'extérieur par
>   `<main_id>.<mesure_id>` — c'est cette adresse qu'un `cout` d'étape de plan cite.
...
- un porteur promu dont la crise est close et qui traîne encore en `orbite` — du budget
  mangé pour rien ;
- une main sans porteur depuis plus de N jours.
...
- **Jusqu'où écrire des mains.** La tentation sera d'en donner à tout le monde. Le critère
  ...
  au bout de combien de lunes sans effet une main se retire du fichier.

---

## Le critère d'écriture

**Une main s'écrit si, et seulement si, elle doit produire un résultat que le joueur
...
2. **Un `cout` qui mord.** Le joueur veut partir et les nefs ne sont pas radoubées ; il veut
   trois cents hommes et le rôle en porte quarante. La main n'a franchi aucun seuil — elle
...
la main ne s'écrit pas** — même si elle serait « réaliste ». Le grain d'un château où le
...
Inversement, une main qui passe le test s'écrit même si elle ne bougera pas de trois

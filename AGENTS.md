...
- **L'excitation a disparu**, et c'est une mesure qui l'a tuée : un compteur qui montait sans sources et retombait d'un point par jour disait qui avait *envie* de parler, jamais qui avait de *quoi*. Ce qui la remplace ne se compte pas — ça se mesure : le **quartier** (qui le joueur peut atteindre à pied) et les **creux** (le temps que sa routine lui laisse, avec la salle où il se tient alors). Un homme sans creux ne pense pas ce jour-là, quelle que soit sa force : c'est le coût d'un mandat, on l'occupe. Voir `docs/boucle-acteurs.md`.

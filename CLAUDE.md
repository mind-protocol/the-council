...
5. Les clics du joueur sont POSTés par la page → `etat/inbox/action-<ts>.json` (`{type: libre|mode|pause|evenement|reaction|pensee|agenda|vue, texte, date_atteinte}`).
   **`vue` — REGARDE L'IMAGE. Tu es aveugle pendant une bataille, et c'est le seul moment où ça compte.** Tant qu'une bataille est dressée, la page compose toutes les 30 secondes le plan, la foule et le fer en une vue **centrée sur le joueur** (260 m de large) et te la dépose : l'entrée d'inbox porte le chemin, jamais l'image. **Ouvre le fichier** — où la ligne a cédé, de quel côté la panique court, quelle rue bouchonne, à combien de pas de la porte se tient l'homme qu'on te fait jouer : ça se voit d'un coup d'œil et ça se raconte mal. Trois cents lignes de relevé ne rendent pas ça.
   - **Les dix dernières sont gardées en rotation**, listées dans `etat/vues/<siège>.json`, la plus récente en tête. `vues[0]` dit où l'on en est ; les suivantes disent le MOUVEMENT — la ligne qui recule de trente mètres entre deux vues est ce qu'aucune vue seule ne peut dire. Cinq minutes de recul à portée de main.
   - **Hors fiction, entièrement.** Ni parole, ni acte, ni minute. Rien à écrire dans `etat/`, rien à répondre au joueur : c'est pour TES yeux, et il ne sait pas que tu regardes. Ce que tu en tires se joue par les voies ordinaires — l'homme qui entre et rapporte ce qu'il a vu, la nouvelle qui arrive, le brouillard qui s'applique comme partout ailleurs. **Ne fais jamais dire à un PNJ ce que tu as lu sur l'image et qu'il n'a pas pu voir de ses yeux.**
   - Plusieurs de ces entrées d'affilée pointent sur la même rotation : elles ne valent que par la plus récente, et l'on n'y répond pas une par une.

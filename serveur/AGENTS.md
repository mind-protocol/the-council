...
`POST /action` persiste l'action et spawn seulement `router_message.py` avec sa `ref`. Le processus Python calcule les présences à la minute du siège : `dire|parler` va directement aux habitants de la même pièce par billet-réveil ; les autres modes vont au MJ. Il écrit le reçu `routage-presence/1`. Le serveur ne doit jamais dupliquer ce routage.
